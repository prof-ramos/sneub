...
const ROASTS = {
  favorable: "Pelo que você marcou, isso não parece uma bosta. E, desta vez, o elogio não vem com asterisco.",
...
const RESULT_COPY = {
  favorable: {
    title: "Tem coisa boa de verdade aqui.",
    lead: (answered, summary) => `Você respondeu ${answered} perguntas. Entre os sinais mapeados, apareceram ${summary.good} favoráveis, ${summary.warn} intermediários e ${summary.bad} de desgaste. O conjunto pende para o que está funcionando — sem transformar isso em prova de perfeição.`
  },
  mixed: {
    title: "Tem coisa boa. Tem coisa pedindo atenção.",
    lead: (answered, summary) => `Você respondeu ${answered} perguntas. Entre os sinais mapeados, apareceram ${summary.good} favoráveis, ${summary.warn} intermediários e ${summary.bad} de desgaste. A leitura é mista: reconhecer o que funciona não exige ignorar o que cobra caro.`
  },
  attention: {
    title: "O que você marcou pede atenção.",
    lead: (answered, summary) => `Você respondeu ${answered} perguntas. Entre os sinais mapeados, apareceram ${summary.good} favoráveis, ${summary.warn} intermediários e ${summary.bad} de desgaste. Há coisa boa aí, mas os sinais de desgaste pesaram mais nesta leitura.`
  },
  unclear: {
    title: "Não vou inventar um veredito.",
    lead: (answered) => `Você respondeu ${answered} perguntas, mas não marcou sinais suficientes para uma leitura equilibrada. Isso é um limite do resultado — não uma sentença sobre a relação.`
  }
};
...
function roastLine(rows, locked, summary = summarizePatterns(rows)) {
  if (locked) return "Este card fica em branco de propósito. Segurança não vira meme.";
  if (!rows.length) return ROASTS.ok;
  if (summary.status === "favorable") return ROASTS.favorable;
  const top = rows[0];
...
function renderOut(customAnalysis = null, analysisWarning = "", options = {}) {
  ...
  const summary = summarizePatterns(rows);
  const copy = RESULT_COPY[summary.status];
  ...
  const line = roastLine(rows, false, summary);
  setChrome("step", "resultado");

  $("out").innerHTML = `
    <div class="count">${answered} perguntas respondidas</div>
    <h1 class="res" id="out-title" tabindex="-1">${copy.title}</h1>
    ...
    <p class="lead">${copy.lead(answered, summary)}</p>
    ${working.length ? `
      <div class="block">
        <div class="k">O que suas respostas validaram</div>
        ${working.map((r) => `<h3>${LABELS[r.k]}</h3><p>Você marcou, com alguma consistência, sinais favoráveis de ${LABELS[r.k].toLowerCase()}. Isso também faz parte da leitura.</p>`).join("")}
      </div>` : ""}
    ...
    <div class="card">
      <div class="tm">Leitura absolutamente não científica™</div>
      <p id="roastcard"></p>
    </div>
